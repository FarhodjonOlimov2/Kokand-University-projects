...
export default function StartScreen() {
  ...
  const directions = [
    ...
    { id: 'psychology', title: t.dir4_title, desc: t.dir4_desc, badge: t.dir4_badge, icon: HeartHandshake, color: '#F43F5E' },
  ...
  return (
    <div className="relative w-full h-screen flex flex-col overflow-hidden bg-[#080C18] select-none">
      ...
      {/* ════════════════════════════════════════════════════════════
           FULL-SCREEN CINEMATIC BACKGROUND — KOKAND PALACE
         ════════════════════════════════════════════════════════════ */}
      <div className="absolute inset-0 z-0">
        {/* Kokand panorama with slow cinematic zoom */}
        <div
          className="absolute inset-0 bg-cover bg-center presidential-slow-zoom"
          style={{ backgroundImage: "url('/images/kokand_bg.jpg')" }}
        />
        {/* Primary cinematic gradient — palace visible through center */}
        <div className="absolute inset-0" style={{
          background: `linear-gradient(180deg, 
            rgba(8,12,24,0.72) 0%, 
            rgba(8,12,24,0.32) 20%, 
            rgba(8,12,24,0.15) 36%,
            rgba(8,12,24,0.40) 56%,
            rgba(8,12,24,0.82) 76%,
            rgba(8,12,24,0.97) 100%
          )`
        }} />
        {/* Radial highlight on palace area */}
        <div className="absolute inset-0" style={{
          background: 'radial-gradient(ellipse at 50% 30%, rgba(255,255,255,0.06) 0%, transparent 55%)'
        }} />
        {/* Cinematic vignette */}
        <div className="absolute inset-0" style={{
          background: 'radial-gradient(ellipse at center, transparent 50%, rgba(8,12,24,0.45) 100%)'
        }} />
      </div>
        ...
        >
          ...
          <div className="flex items-center gap-3">
            <div
              ...
              style={{
                background: 'linear-gradient(135deg, #0B6E4F, #0D9488)',
            ...
            <div className="hidden sm:block">
              ...
              <p className="text-[9px] text-white/30 font-semibold mt-0.5 tracking-wide">{t.startSubBrand}</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2.5">
              ...
              {(['UZ', 'RU', 'EN'] as const).map((lang) => (
                <button
                  ...
                  className={`px-2.5 py-1.5 text-[11px] font-black rounded-lg transition-all duration-300 cursor-pointer ${
                    language === lang
                      ? 'bg-white text-[#0A0F1E] shadow-md'
            ...
            >
              ...
              <span className="text-[10px] font-black text-emerald-400/80 tracking-wider">{t.startKioskBadge}</span>
            </div>
          </div>
        </nav>
      </header>
      ...
      <main className="relative z-20 flex-1 flex flex-col items-center justify-center px-4 -mt-6">
        ...
        <div className={`transition-all duration-1000 delay-300 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
          <div
            ...
            style={{
              background: 'linear-gradient(135deg, rgba(212,175,55,0.12), rgba(180,150,40,0.05))',
              border: '1px solid rgba(212,175,55,0.25)',
            }}
          >
            <Sparkles className="w-3.5 h-3.5" style={{ color: '#D4AF37' }} />
            <span className="text-[11px] font-black tracking-[0.15em] uppercase" style={{ color: 'rgba(212,175,55,0.85)' }}>
              {t.startProjectTitle}
            </span>
          </div>
        </div>
        ...
        <div className={`text-center transition-all duration-1000 delay-500 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          <h1 style={{ fontSize: 'clamp(2.6rem, 6.5vw, 5.5rem)', fontWeight: 900, letterSpacing: '-0.035em', lineHeight: 0.95 }}>
            <span className="text-white">{t.startMainTitle1}</span>
            <span
              style={{
        ...
        <div className={`transition-all duration-1000 delay-700 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
          <p
            className="text-white/40 max-w-xl mx-auto text-center mt-5 mb-7"
            style={{ fontSize: 'clamp(0.85rem, 1.4vw, 1.1rem)', fontWeight: 500, lineHeight: 1.7 }}
          >
            {t.startMotto}
          </p>
        </div>
        ...
        <div className={`transition-all duration-1000 delay-[900ms] ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
          <div className="relative">
            ...
            <div
              className="absolute -inset-3 rounded-[22px] pointer-events-none presidential-breathe"
              style={{ background: 'linear-gradient(135deg, #10B981, #0D9488)', filter: 'blur(22px)' }}
            />
            <button
              onClick={handleStart}
              className="relative group cursor-pointer active:scale-[0.97] transition-all duration-300 hover:-translate-y-1 flex items-center justify-center gap-3"
              style={{
                minHeight: '68px',
                minWidth: '320px',
                padding: '18px 48px',
                ...
                color: '#FFF',
                background: 'linear-gradient(135deg, #0B6E4F, #0D9488, #0891B2)',
                border: '1px solid rgba(255,255,255,0.15)',
                borderRadius: '20px',
                boxShadow: '0 20px 50px rgba(11,110,79,0.3), 0 4px 12px rgba(0,0,0,0.15), inset 0 1px 0 rgba(255,255,255,0.1)',
              }}
            >
              <Sparkles className="w-5 h-5 text-white/60 group-hover:rotate-12 transition-transform duration-300" />
              <span>{t.startButton}</span>
              <ArrowRight className="w-5 h-5 text-white/60 group-hover:translate-x-1.5 transition-transform duration-300" />
            </button>
          </div>
        </div>

        {/* Touch Instruction */}
        <p className={`mt-3.5 text-[11px] font-bold tracking-[0.2em] uppercase transition-all duration-1000 delay-[1100ms] ${mounted ? 'opacity-100' : 'opacity-0'}`}
