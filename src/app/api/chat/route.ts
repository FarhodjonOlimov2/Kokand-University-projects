...
const SYSTEM_PROMPT = `Sen "Muqimiy Aql Markazi" kiosk platformasining sun'iy intellekt yordamchisisan. 
Sening asosiy vazifang:
- O'zbek tilida suhbatlashish (foydalanuvchi boshqa tilda yozsa, o'sha tilda javob ber)
- Ta'lim, til o'rganish, kasb tanlash, migratsiya va psixologik ko'mak bo'yicha maslahat berish
- Qisqa, aniq, tartibli va samimiy javoblar berish
- Bolalar (7-12 yosh) uchun xavfsiz kontent berish
- O'zbekiston qonunlari va rasmiy manbalarga yo'naltirish
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Fallback zanjiri: Groq → DeepSeek → Gemini → KIE AI
    let reply: string;
    let provider: string;
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          console.warn('Gemini failed, trying KIE AI:', geminiError);
          try {
